...
//    public double lowTurnPos, lowBonkPos, lowRotPos, lowClawPos, highTurnPos, highBonkPos, highClawPos;
    ...
    private final Pose pickUp1 = new Pose(35, 19.5, Math.toRadians(0));
    private final Pose drop1 = new Pose(21, 16.5, Math.toRadians(0));
    private final Pose pickUp2 = new Pose(35, 10, Math.toRadians(0));
    private final Pose drop2 = new Pose(21, 8, Math.toRadians(0));
    private final Pose pickUp3 = new Pose(32, 6, Math.toRadians(0));
    private final Pose drop3 = new Pose(20, 13.5, Math.toRadians(0));
    ...
    private PathChain score1, sample1, sampleDrop1, sample2, sampleDrop2, sample3, sampleDrop3, speciminGrab1, score2, speciminGrab2, score3, speciminGrab3, score4, speciminGrab4, score5, toPark;
    ...
    public void buildPaths() {
        ...
        sample1 = follower.pathBuilder().addPath(
                new BezierLine(
                        new Point(specimen1),
                        new Point(pickUp1)
                )).setConstantHeadingInterpolation(0).build();
        ...
        sampleDrop1 = follower.pathBuilder().addPath(
                new BezierLine(
                        new Point(pickUp1),
                        new Point(drop1)
                )).setConstantHeadingInterpolation(Math.toRadians(0)).build();

        sample2 = follower.pathBuilder().addPath(
                new BezierLine(
                        new Point(drop1),
                        new Point(pickUp2)
                )).setConstantHeadingInterpolation(Math.toRadians(0)).build();

        sampleDrop2 = follower.pathBuilder().addPath(
                new BezierLine(
                        new Point(pickUp2),
                        new Point(drop2)
                )).setConstantHeadingInterpolation(Math.toRadians(0)).build();

        sample3 = follower.pathBuilder().addPath(
                new BezierLine(
                        new Point(drop2),
                        new Point(pickUp3)
                )).setConstantHeadingInterpolation(Math.toRadians(0)).build();
...
//                )).setConstantHeadingInterpolation(0).build();
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case (5):
                if (pathTimer.getElapsedTime() > 250) {
                    //start to extend anvil and get ready to pick up the first sample
                    if(!follower.isBusy()) {
                        //low bonk go down
                        setPathState(6);
                    }
            ...
            case (7):
                if (pathTimer.getElapsedTime() > 50) {
                    ...
                    highBonkPos = 1;
                    highClawPos = 0;
                    follower.followPath(sampleDrop1);
                    setPathState(8);
                }
                break;
            case (8):
            ...
            case (12):
                if (pathTimer.getElapsedTime() > 50) {
                    ...
                    //start to extend the anvil and get the hammer ready for transfer
                    setPathState(13);
                }
                break;
            case (13):
            ...
            case (15):
                if (pathTimer.getElapsedTime() > 50) {
                    ...
                    highBonkPos = 1;
                    highClawPos = 0;
                    follower.followPath(sampleDrop2);
                    setPathState(16);
                }
                break;
            case (16):
